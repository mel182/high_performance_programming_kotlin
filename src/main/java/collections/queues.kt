...
fun main() {
    ...
    if (names.isNotEmpty())
    {
        ...
        println("queue: ${names}")
        val elementFound = names.element()
    ...
    if (names.isNotEmpty())
    {
        ...
        println("Element found using element: ${elementFound}")
        println("Element found using peek: ${names.peek()}")
        names.poll() // The poll() method is similar to remove() (dequeue operation),
        // but it returns null if the Queue is empty instead of throwing an exception

        val melchior3 = "Melchior3"
        if (names.contains(melchior3))
            println("Queue contains $melchior3 at index: ${names.indexOf(melchior3)}")
